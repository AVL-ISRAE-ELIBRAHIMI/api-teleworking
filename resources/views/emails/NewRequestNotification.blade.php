    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px; margin:auto; background:#ffffff; border-radius:8px; overflow:hidden; box-shadow:0 2px 6px rgba(0,0,0,0.1);">
        ...
        <tr>
            <td style="padding:20px; color:#333;">
                ...
                <p>Bonjour <strong>{{ $responsable->name }}</strong>,</p>
                ...
                <table width="100%" cellpadding="8" cellspacing="0" style="border:1px solid #ddd; border-radius:6px; margin-top:10px;">
                    <tr style="background:#f9f9f9;">
                        ...
                        <td>{{ $materiel->label }}</td>
                    </tr>
                    <tr>
                        <td><strong>Projet</strong></td>
                        <td>{{ $project->code_projet }}</td>
                    </tr>
                    <tr style="background:#f9f9f9;">
                        <td><strong>Demandeur</strong></td>
                        <td>{{ $requestor->nom }} {{ $requestor->prenom }}</td>
                    </tr>
                    <tr>
                        <td><strong>Date de début</strong></td>
                        <td>{{ $requestAsset->borrow_date }}</td>
                    </tr>
                    <tr style="background:#f9f9f9;">
                        <td><strong>Date de retour</strong></td>
                        <td>{{ $requestAsset->return_date }}</td>
                    </tr>
                    <tr>
                        <td><strong>Lieu prévu</strong></td>
                        <td>{{ $requestAsset->new_location }}</td>
                    </tr>
                </table>
                <div style="text-align:center; margin-top:22px;">
                    <a href="{{ $requestUrl }}" style="display:inline-block; background:#4caf50; color:#ffffff; text-decoration:none; padding:12px 28px; border-radius:6px; font-weight:bold; font-size:14px;">
                        Voir la demande
                    </a>
                </div>
            </td>
        ...
        <tr>
            <td style=" color:#0d3b66; text-align:center; padding:15px;">
                <small>&copy; {{ date('Y') }} AVL TOOLS CENTER</small>
            </td>
        </tr>
    </table>
